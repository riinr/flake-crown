...
proc inputInfo(refInfo: JsonNode, url: string): JsonNode =
  ...
  for nimbleUrl in nimbleUrls:
    let 
      ...
      owner    = repoInfo[1]
      repo     = repoInfo[2]
      url      = fmt"git+{nimbleUrl}"
      repoType = if repoInfo[0].contains "github":
                   "github"
                 elif repoInfo[0].contains "gitlab":
                   "gitlab"
                 elif repoInfo[0].contains "git.sr.ht":
                   "sourcehut"
                 else:
                   "git"
      refs     = if repoType == "sourcehut":
                   refInfo["ref"].getStr.split("/")[^1]
                 else:
                   refInfo["ref"].getStr
...
{nimPkgsLibInput()}{inputs}
  ...
  in lib.mkRefOutput {{
    ...
    fmt"{flakeDir}/meta.json".writeFile $refInfo
    discard execCmd fmt"""
      cd {flakeDir};
      git add .
...
{nimPkgsLibInput()}{heads}{tags}
  ...
  fmt"../nimpkgs/{flakeDir}/flake.nix".writeFile flakeContent
  fmt"../nimpkgs/{flakeDir}/meta.json".writeFile $pkg
  discard execCmd fmt"""
    cd ../nimpkgs/{flakeDir};
    git init &>/dev/null
    git add .
