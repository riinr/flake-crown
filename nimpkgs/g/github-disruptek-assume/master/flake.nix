{
  ...
  inputs.src-assume-master.flake = false;
  inputs.src-assume-master.ref   = "refs/heads/master";
  inputs.src-assume-master.owner = "disruptek";
  inputs.src-assume-master.repo  = "assume";
  inputs.src-assume-master.type  = "github";
  ...
  let 
    ...
    args = ["self" "nixpkgs" "flakeNimbleLib" "src-assume-master"];
    over = if builtins.pathExists ./override.nix 
           then { override = import ./override.nix; }
           else { };
  in lib.mkRefOutput (over // {
    inherit self nixpkgs ;
    src  = deps."src-assume-master";
